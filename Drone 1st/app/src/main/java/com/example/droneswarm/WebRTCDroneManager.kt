...
// ─────────────────────────────────────────────────────────────────────────────
// H264OnlyVideoEncoderFactory
// DefaultVideoEncoderFactory ko wrap karta hai aur H265/HEVC codec
// getSupportedCodecs() se filter kar deta hai.
// Isse drone ka SDP offer pehle se H265-free hoga — server ko kuch
// patch nahi karna padega (double safety layer).
// ─────────────────────────────────────────────────────────────────────────────
private class H264OnlyVideoEncoderFactory(
    ...
    override fun createEncoder(info: VideoCodecInfo): VideoEncoder? {
        return if (isH265(info)) null else delegate.createEncoder(info)
    }
...
class WebRTCDroneManager(
    ...
    private fun initializeFactory() {
        try {
            ...
            // H264OnlyVideoEncoderFactory — H265 SDP offer mein aayega hi nahi
    ...
    private fun setupVideoTrack() {
       ...
       // cameraXCapturer?.startCapture(640, 480, 30)
    ...
    fun handleRemoteAnswer(sdpString: String) {
        ...
        peerConnection?.setRemoteDescription(object : SdpObserver {
            ...
            override fun onSetFailure(err: String?) {
                Log.e(TAG, "❌ Remote answer set failed: $err")
            }
            override fun onCreateSuccess(p0: SessionDescription?) {}
            override fun onCreateFailure(p0: String?) {}
        }, sdp)
